...
class TimerViewController: NSViewController {
  ...
  func createMainTimers() {
    workTimer = Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(TimerViewController.workUpdate), 
      userInfo: nil, repeats: true)
    soundTimer = Timer.scheduledTimer(timeInterval: TimeInterval(count), target: self, selector: 
      #selector(TimerViewController.endWorkTimer), userInfo: nil, repeats: false)
  }

  // Update GUI while workTimer is running
  @objc func workUpdate() {
    if(count > 0) {
      count = count - 1
  ...
  @objc func endWorkTimer() {
    showEndMainNotification()
    workTimer.invalidate()
    soundTimer.invalidate()
    timeLabel.stringValue = "0 min"
    count = 21
    breakTimer = Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(TimerViewController.breakUpdate), 
      userInfo: nil, repeats: true)
    breakSoundTimer = Timer.scheduledTimer(timeInterval: TimeInterval(count), target: self, selector: 
      #selector(TimerViewController.endBreakTimer), userInfo: nil, repeats: false)
  }

  // End and invalidate breakTimer
  @objc func endBreakTimer() {
